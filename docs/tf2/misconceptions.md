...
* `mem_max_heapsize 2048` : No need to adjust the cap for the heap to be wildly large, because the memory system initializes before this variable is applied. Source already determines the best value from your memory size and caps it using this console variable. If it did work, setting it high would [take away memory](https://github.com/ValveSoftware/Source-1-Games/issues/1543#issuecomment-520534294) from other things, and also increase pause times for memory management.
