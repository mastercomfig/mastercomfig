...
Если вы хотите полностью отключить запуск модуля, вы можете поместить (например) `alias textures` в `user / modules.cfg`. Это предотвратит запуск модулей текстур.Исключением для этого правила является псевдоним `snapshot_buffer`. Вы должны также использовать `alias net_projectiles;alias net_hitscan;alias net_spy`, чтобы полностью отключить модуль `snapshot_buffer` .
...
Настройка по умолчанию: **`snapshot_buffer=low`** (все пресеты, _включая None_).
...
Настройка по умолчанию: **`bandwidth=762Kbps`** (все пресеты, _включая None_).
...
Настройка по умолчанию: **`hud_achievement=on`** (все пресеты, _включая None_, кроме Very Low).
...
Настройки по умолчанию: **`move_bind=wasd`** (все пресеты, _включая None_).
