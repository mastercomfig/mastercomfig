...
Настройка по умолчанию: **`snapshot_buffer=safe`** (все пресеты).
...
* **`snapshot_buffer=low`**: Самый быстрый вариант с минимальным временем буферизации для защиты (**15 мс lerp** при использовании `packet_rate=standard`, или **30 мс lerp** при использовании `packet_rate=congestion`).
* **`snapshot_buffer=safe`**: самый безопасный вариант с минимальной задержкой снимка при использовании каждого второго снимка (**30 мс lerp** при использовании `packet_rate=standard` или **60 мс lerp** при использовании `packet_rate=congestion`).
* **`snapshot_buffer=high`**: Сильно защищает от потери пакетов, используя каждый третий снимок (**45 мс lerp** при использовании `packet_rate=standard`, или **90 мс lerp** при использовании `packet_rate=congestion` ).
