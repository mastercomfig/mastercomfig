...
Настройка по умолчанию : **`romevision=off`** (все предустановки, кроме Ультра, Высокая и Нет).
...
!!! Внимание
    В пресете «Очень низкий» декали принудительно невидимы.
...
Настройка по умолчанию: **`hud_panels=high`** (все пресеты, кроме Низкий и Очень Низкий).
...
Настройка по умолчанию: **`hud_avatars=friends`** (все пресеты, кроме Низкий и Очень Низкий).
...
Настройки по умолчанию: **`logo=on`** (все пресеты, кроме Очень Низкий).
...
!!! Внимание
   Этот модуль должен быть установлен в `pre_comfig.cfg` вместо `modules.cfg`.

Управляет выводом языка mastercomfig.

Настройки по умолчанию: **`lang=en`** (все персеты).

* **`lang=en`**: English (Английский)
* **`lang=pt`**: Português (Португальский)
* **`lang=ru`**: Pусский (Russian)
* **`lang=es`**: Español (Испанский)
* **`lang=fr`**: Français (Французский)
* **`lang=hr`**: Hrvatski (Хорватсий)
* **`lang=cs`**: Česká (Чешский)
