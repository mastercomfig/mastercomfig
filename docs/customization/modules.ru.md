...
Настройки по умолчанию: **`sillygibs=automatic`** (все пресеты).

* **`sillygibs=automatic`**: Автоматически выбирает лучшие настройки для производительности (`on` если расчленение отключено, `off` если расчленение включено).
