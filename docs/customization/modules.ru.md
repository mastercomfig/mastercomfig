...
* **`sprays=on`**: Отображает спреи, и полностью блокирует их периодическое удаление выполняемую mastercomfig. Если модуль `decals` имеет значение `off`, он будет установлен на `low` уровень.
