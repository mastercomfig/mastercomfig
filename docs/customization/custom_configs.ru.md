...
* `shadows`: Uses `shadowlod_override` для управления качеством моделей теней
...
* `water`: Использует `cheap_water_override` для контроля расстояния затухания воды низкого качества
  ...
  * `cheap_water_partial`: Всегда использует частично низкокачественную воду, использует полностью низкокачественную воду в близи расстоянии
  * `cheap_water_minimal`: Использует частично низкокачественную воду в близи расстоянии, использует полностью низкокачественную воду на расстоянии
...
* `props`: Использует `detail_props_override` для управления тона травы/детали рендеринга prop и расстояние отрисовки 
  * `detail_props_none`: Отключftn детали props 
