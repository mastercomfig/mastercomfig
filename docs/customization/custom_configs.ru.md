...
Имя `overrides/autoexec` можно изменить с помощью псевдонима `autoexec_c`, а имя `overrides/modules.cfg` с помощью псевдонима `modules_c`.
...
Примерами использования `overrides/pre_comfig.cfg` могут быть такие, как настройка предустановки, как подробно описано выше, или включение параметров отладки для мониторинга процесса запуска самого mastercomfig.
