...
Если вы хотите переопределить эту часть модуля, вам придется использовать его псевдоним переопределения игры в `tf/cfg/user/ autoexec.cfg`. Это состоит из
...
Вы можете изменить имя своей пользовательской конфигурации локального сервера, используя псевдоним `listenserver_c`.
Например, вы можете поместить `alias listenserver_c"exec user/mylistenserver.cfg"` в `user/autoexec.cfg`, если хотите использовать файл `tf/cfg/user/mylistenserver.cfg`.
...
Примерами использования `user/pre_comfig.cfg` могут быть такие, как настройка предустановки, как подробно описано выше, или включение параметров отладки для мониторинга процесса запуска самого mastercomfig (например: `sv_cheats 1;mat_debugalttab 1`).
