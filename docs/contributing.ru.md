...
или с идеей для нововведения, именно вы можете подсказать, в какую сторону двигаться mastercomfig,
чтобы он идеально подходил вам и при этом был лучше для всех!
...
Вы также можете взглянуть на [полную папку с исходниками](https://github.com/mastercomfig/mastercomfig/tree/release/docs) чтобы добавить новые файлы или осмотреться.
...
Вы можете помочь пользователям, которые задают вопросы в [Discord](https://comfig.app/discord).
...
- `no-pyroland`: Удалены текстуры карты Pyroland
...
- Добавьте псевдонимы на уровне модуля (модулей). (`alias module_option "cvar1 1;cvar2 0;alias module_level echo module=option"`). Для каждой команды в модуле все уровни должны установить эту команду, если только она не влияет на данный уровень.
- Добавьте псевдоним (псевдонимы) уровня установленного модуля. (`alias module=option"alias module module_option""`).
- Возможно, настроить пресеты в `config/cfg/presets` чтобы использовать новый модуль или уровни для существующего модуля.

Если вы добавляете новый модуль, вам также нужно будет добавить новую запись в `module` внутри `config/mastercomfig/cfg/comfig/modules_run.cfg` и `config/mastercomfig/cfg/comfig/modules_levels.cfg`.
