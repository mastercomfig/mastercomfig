...
Как правило, думайте о mastercomfig как о "слоях", а не о том, что вам нужно редактировать напрямую. Ваш пользовательский уровень состоит из слоя пользовательских модулей, слоя autoexec, слоя переопределений игры и слоя конфигурации класса. Модули перезаписывают уровень модулей mastercomfig/preset. Уровень Autoexec перезаписывает слой mastercomfig/addons. Слой переопределений игры перезаписывает переопределения игры mastercomfig (запускается при присоединении к игре/каждый раз, когда вы меняете класс) и сетевые настройки для каждого класса, для всех классов. Настройки для конкретного класса вступают в силу поверх глобальных настроек mastercomfig (так как загружаются после них) и меняют сетевые настройки только для этого класса.

Такой многослойный подход означает, что настройки конфига становятся более упорядоченными (то есть большая часть работы уже сделана за вас, а вам остаётся лишь изменить несколько параметров под свои предпочтения, чтобы избежать конфликтов и обеспечить максимальную стабильность и производительность), а также изолированными (вы можете быстро установить новую версию mastercomfig, а ваши пользовательские надстройки останутся в папке overrides, поэтому при смене версии вам не придётся ничего перенастраивать заново). Дополнительная информация приведена в следующих разделах.
