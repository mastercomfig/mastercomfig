...
## "Unknown command gl_\*" в консоли

mastercomfig устанавливает некоторые связанные с OpenGL консольные переменные, которые доступны только в Linux и macOS. Таким образом, эти ошибки появляются несколько раз при использовании mastercomfig в Windows. Эти ошибки безвредны и их можно безопасно игнорировать. Однако можно отключить выполнение команд, поместив `opengl=skip` в ваш `modules.cfg`.
